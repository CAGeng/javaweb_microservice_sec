...
import com.l4yn3.microserviceseclab.mapper.IStudent2Mapper;
...
public class IndexController {
    ...
    @Autowired
    IStudent2Mapper iStudent2Mapper;
    ...
    // 不是漏洞
    @RequestMapping(value = "/one_sftsafe")
    public List<Student> one_sftsafe(@RequestParam(value = "username") String username) {
        return indexLogic.getStudentsft(username);
    }
    ...
    @RequestMapping(value = "/myBatisSft")
    public List<Student2> myBatisSft(@RequestParam(value = "name") String name) {
        return iStudent2Mapper.query2(name);
    }
    ...
    // 正确写法-sft
    @RequestMapping(value = "/jpaWithAnnotationssft")
    public List<Person> jpawithAnnotations_sft(@RequestParam(value = "name") String name) {
        return personRepository.findPersonByNickname_good(name);
    }
}
